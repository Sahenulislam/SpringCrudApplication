...
public interface StudentRepository extends JpaRepository<Student, Long> {
   ...
   List<StudentLaptopProjection> findAllLaptopByUser(@Param("id") Long id);
   @Query(value = "SELECT s.id as studentId, s.name as studentName, s.intake, s.section, l.id as laptopId, l.company as laptopCompany " +
           "FROM student s " +
           "INNER JOIN laptop l ON s.id = l.student_id ", nativeQuery = true)
   List<StudentLaptopProjection>findAllLaptopByAllUser();
